...
class Edge:
    ...
    def change(self, inedge, outedge, **kwargs):
        self.inedge = inedge
        self.weight = kwargs.get('weight')
        #Прикручиваем событие (скорее всего функция)
        self.action = kwargs.get('action')
        self.label = kwargs.get('label')
        self.password = kwargs.get('password')
        self.outedge.append(outedge)
...
class Graph(AbstractGraph):
    ...
    #Добавить вершину и в случае её отсутствия, выбросить исключение
    ...
    def set_width(self, edge_in, ed, wid):
        self.graphbase[edge_in] = Edge(edge_in, ed, weight=wid)
